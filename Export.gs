...
/**
 * Fungsi Backend untuk membuat PDF Tabel Landscape
 */
function apiGenerateTablePdf(dataArray, filterName) {
  try {
    const template = HtmlService.createTemplateFromFile('report_table_template');
    template.data = dataArray;
    template.tgl_cetak = Utilities.formatDate(new Date(), "GMT+7", "dd MMMM yyyy HH:mm");
    template.filter = filterName;

    const htmlOutput = template.evaluate().getContent();
    const pdf = Utilities.newBlob(htmlOutput, 'text/html', 'temp_table.html')
                 .getAs('application/pdf')
                 .setName("REKAP_MONITORING_LKD_" + filterName + ".pdf");
